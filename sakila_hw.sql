...
ALTER TABLE actor MODIFY COLUMN last_name VARCHAR(50) AFTER middle_name;
...
-- currently `HARPO`, change it to `GROUCHO`. Otherwise, change the first name 
-- to `MUCHO GROUCHO`, as that is exactly what the actor will be with the grievous error. 
...
GROUP BY fa.film_id;
-- testing...the below query should give 11
-- SELECT COUNT(actor_id) FROM film_actor WHERE film_id = 827;
...
-- testing....the below query should give 6
...
-- testing....the below query should give 93.76
-- SELECT SUM(amount) FROM payment WHERE customer_id = 130;
...
-- testing....the below query should give 24
-- SELECT COUNT(rental_id) FROM rental WHERE inventory_id IN (1384, 1385, 1386, 1387, 1388, 1389);
...
DROP VIEW top5_genres_gross_revenue;
